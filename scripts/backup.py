...
def create(out_dir: Path, include_env: bool, include_data: bool, settings: Settings) -> Path:
    ...
    entries: list[tuple[str, Path]] = [(f"storage/{p.relative_to(storage).as_posix()}", p) for p in _files(storage)]
    if include_data:
...
def restore(archive: Path, force: bool, settings: Settings) -> None:
    ...
    storage, data = Path(settings.storage_dir), Path(settings.data_dir)
    if not force and any(storage.glob("*")):
        raise SystemExit(f"{storage} is not empty. Stop the app and re-run with --force to overwrite.")
    with tarfile.open(archive, "r:gz") as tar:
        for m in _safe_members(tar):
            if m.name == "manifest.json" or not m.isfile():
                continue
            if m.name.startswith("storage/"):
                dest = storage / m.name[len("storage/"):]
            elif m.name.startswith("data/"):
                dest = data / m.name[len("data/"):]
            elif m.name == ".env":
                dest = ROOT / ".env"
            else:
                continue
